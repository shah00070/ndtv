...
import com.ndtv.core.util.BitmapCacheAppWidget;
...
public class ImageLoader extends com.android.volley.toolbox.ImageLoader {
    ...
    private static final int HALF_FADE_IN_TIME = Utility.ANIMATION_FADE_IN_TIME / 2;
    ...
    public ImageLoader(Context context) {
        super(newRequestQueue(context), new BitmapCacheAppWidget(
                BitmapCacheAppWidget.calculateMemCacheSize(BitmapCacheAppWidget.DEFAULT_MEM_CACHE_PERCENT)));
    }
        ...
        if (requestUrl == null || !requestUrl.equals(recycledImageUrl)) {
            ...
            if (requestUrl != null) {
                ...
                imageContainer = get(requestUrl, getImageListener(mResources, imageView, placeHolder, mFadeInImage),
                        maxWidth, maxHeight/*, imageView.getContext().getApplicationContext()*/);
...
/*		// On HC+ use HurlStack which is based on HttpURLConnection. Otherwise
		...
		Network network = new BasicNetwork(Utility.hasHoneycomb() ? new HurlStack() : new HttpClientStack(
		...
		return queue;
		*/        //formatter:on
        return NdtvApplication.getApplication(context.getApplicationContext()).mRequestQueue;
    }
    ...
    private static void setImageBitmap(final ImageView imageView, final Bitmap bitmap, Resources resources,
        ...
        if (fadeIn) {
            ...
            td.startTransition(Utility.ANIMATION_FADE_IN_TIME);
        } else {
    ...
    public static File getDiskCacheDir(Context context, String uniqueName) {
        ...
        final String cachePath = Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                || !Environment.isExternalStorageRemovable() ? getExternalCacheDir(context).getAbsolutePath() : context
                .getCacheDir().getPath();
